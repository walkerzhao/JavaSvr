package com.tencent.dolphin.svrcore.conf;
...
//        init();
    ...
    public JungleConfiguration(Injector injector) {
        ...
        if (hasServletContext) {
            try {
                ...
                // no com.tencent.dolphin.webdemo.servlet context found
    ...
    String init() {
        ...
        log("Dolphin app name is " + appName);


        // 环境变量最优先： 次序为<basename>前缀，dolphin前缀，无前缀
        ...
        addConfiguration(new SubsetConfiguration(envconf, "dolphin"));
        log("add env config subset[dolphin]");
        addConfiguration(envconf);
        log("add env config");
        
        // 文件系统中，次序为
        //   ../conf/dolphin.properties
        //   ${JUNGLE_SPEC_CONF}/dolphin.properties
        //   $HOME/.<basename>.properties
        //   /etc/dolphin/<basename>.properties
        //   $HOME/.dolphin.properties
        //   /etc/dolphin/dolphin.properties
        if(!hasServletContext)
        {
           log("user.dir is"+System.getProperty("user.dir"));
           addConfigURL("../conf/dolphin.properties");
        }
        //
        String specConf = System.getProperty("JUNGLE_SPEC_CONF");
        log("JUNGLE_SPEC_CONF path: " + specConf);
        addConfigURL(specConf + "/dolphin.properties");
        
        if (appName != null) {
            addConfigURL(homeDir + "/." + appName + ".properties");
            addConfigURL("/etc/dolphin/" + appName + ".properties");
        }
        addConfigURL(homeDir + "/dolphin.properties");
        addConfigURL("/etc/dolphin/dolphin.properties");
        ...
        addConfigURL("webapp:dolphin.properties");

        // 最后是classpath下的文件
        if (appName != null)
            addConfigURL("classpath:" + appName + ".properties");
        addConfigURL("classpath:dolphin.properties");
        addConfigURL("./dolphin.properties");

        if (hasServletContext && servletContext != null) {
            try {
                addProperty("dolphin.webapp.name", MethodUtils.invokeExactMethod(servletContext, "getContextPath", new Object[]{}));
                addProperty("dolphin.webapp.baseDir", MethodUtils.invokeExactMethod(servletContext, "getContextPath", "/"));
            } catch (Exception e) {
                // ignore
            }
        }
    ...
    private String getAppName() {
        ...
        if ("dolphin".equals(appName)) // jungle名总是要加载
            appName = null;
        return appName;
    }
